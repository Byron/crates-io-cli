...
fn show_changes_new(repo_path: PathBuf, output_kind: OutputKind) -> Result<(), Error> {
...
pub fn handle_recent_changes_new(
    ...
    let computation = std::thread::spawn({
        ...
        move || show_changes_new(repo_path, output_format)
    });
